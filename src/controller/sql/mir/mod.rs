...
use nom_sql::{LimitClause, OrderClause, SelectStatement, CompoundSelectOperator};
...
impl SqlToMirConverter {
    ...
    pub fn named_query_to_mir(
        ...
        let nodes = self.make_nodes_for_selection(&name, sq, qg, is_leaf, universe);
    ...
    fn union_grouped_results(
        &mut self,
        st: &SelectStatement,
        qg: &QueryGraph,
        prev_node: &mut MirNodeRef,
                ...
                if !gb_edges.is_empty() {
                    ...
                    for fn_col in &computed_cols_cgn.columns {
                        ...
                        let new_fn_col = &Column {
                            ...
                            name: fn_col.name.clone(),
                            alias: fn_col.alias.clone(),
                            table: fn_col.table.clone()
                        };
                        ...
                        let parent_node = prev_node.clone();
    ...
    fn make_linear_query(
        ...
        new_node_count: &mut usize,
        uname: &str,
    ) -> (Vec<MirNodeRef>, Vec<MirNodeRef>, Vec<MirNodeRef>) {
            ...
            Some(computed_cols_cgn) => {
                ...
                for ccol in &computed_cols_cgn.columns {
                    ...
                    if column_to_predicates.contains_key(&over_col) {
                        ...
                        *prev_node = new_mpns.last().unwrap().clone();
        ...
        for rel in &sorted_rels {
            ...
            if !qgn.predicates.is_empty() {
                ...
                for (i, ref p) in qgn.predicates.iter().enumerate() {
                    ...
                    *prev_node = fns.iter().last().unwrap().clone();
                    predicate_nodes.extend(fns);
                }
            }
        }

        (func_nodes, predicate_nodes, predicates_above_group_by_nodes)
    ...
    fn make_nodes_for_selection(
        ...
        let uname = if universe == "global".into() {
            String::from("")
        } else {
            format!("_u{}", universe)
        };
        ...
        // (Base, Join, GroupBy, Filter, Project, Reader)
        {
            ...
            let mut join_nodes: Vec<MirNodeRef> = Vec::new();
            {
                ...
                for jref in qg.join_order.iter() {
                    ...
                    let jn = self.make_join_node(
                        &format!("q_{:x}_n{}{}", qg.signature().hash, new_node_count, uname,),
                        jp,
            ...
            let mut prev_node = match join_nodes.last() {
                ...
                None => {
                    ...
                    base_nodes.last().unwrap().clone()
                }
            };

            use controller::sql::mir::security::SecurityBoundary;
            info!(self.log, "new node count is {}", new_node_count);
            let mut policy_nodes =
                self.make_security_boundary(universe.clone(), &mut node_for_rel, prev_node.clone(), st, qg, new_node_count);

            new_node_count += policy_nodes.len();
            let (mut func_nodes, predicate_nodes, predicates_above_group_by_nodes) = if let Some(n) = policy_nodes.last() {
                prev_node = n.clone();
                let ugn = if qg.relations.get("computed_columns").is_some() {
                    self.union_grouped_results(st, qg, &mut prev_node, &mut new_node_count, &uname)
                } else {
                    Vec::new()
                };
                (ugn, Vec::new(), Vec::new())
            } else {
                self.make_linear_query(st, qg, &mut prev_node, &mut new_node_count, &uname)
            };
            ...
            let mut final_node: MirNodeRef = prev_node.clone();
            ...
            debug_assert_eq!(
                new_node_count,
                join_nodes.len() + func_nodes.len() + predicate_nodes.len() + policy_nodes.len()
            );
            ...
            let ident = format!("q_{:x}_n{}{}", qg.signature().hash, new_node_count, uname);
