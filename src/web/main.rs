...
fn main() {
    ...
    let article = g.incorporate(new("article", &["id", "title", "url"], true, Base {}), vec![]);
    ...
    // emit first, second, and third field from article (id + title + url)
    // and second field from right (votes)
    let emit = vec![(article, 0), (article, 1), (article, 2), (vc, 1)];
    ...
    g.incorporate(new("awvc", &["id", "title", "url", "votes"], true, j),
