...
mod tests {
    ...
    fn setup(mat: bool, wide: bool) -> ops::Node {
        ...
        g[s].as_ref().unwrap().process((vec![1.into(), 4.into()], 0).into(), s, 0);
        ...
        g[s].as_ref().unwrap().process((vec![1.into(), 7.into()], 4).into(), s, 4);
        g[s].as_ref().unwrap().process((vec![1.into(), 0.into()], 5).into(), s, 5);
        node
    }
    ...
    fn it_forwards() {
        ...
        let out = c.process((vec![1.into(), 2.into()], 2).into(), src, 2);
        assert!(out.is_none());
        ...
        // One more new max.
        let out = c.process((vec![1.into(), 22.into()], 5).into(), src, 5);
