...
pub fn create_n_users(backend: &mut Backend, n: i32) {
    let mut users: Vec<Vec<DataType>> = vec![];
    for i in 0..n {
        let v = vec![
            format!("u{}", i).into(),
            "email".into(),
...
pub fn create_papers_coauthors(backend: &mut Backend, nauthors: i32) {
    // Paper
    // id int,
    // author varchar(1024),
    // accepted tinyint(1),
    let mut papers: Vec<Vec<DataType>> = vec![];
    for i in 0..nauthors {
        let v = vec![i.into(), format!("u{}", i).into(), 0.into()];
        papers.push(v);
    }
    ...
    // paper int,
    // author varchar(1024),
    let mut paper_coauthors: Vec<Vec<DataType>> = vec![];
    for i in 0..nauthors {
        let v = vec![((i + 1)%(nauthors - 1)).into(), format!("u{}",i).into()];
        paper_coauthors.push(v);
    }
    
    let mut mutator = backend.g.table("Paper").unwrap().into_sync();
    println!("inserting papers");
    mutator.perform_all(papers).unwrap();

    let mut mutator = backend.g.table("PaperCoauthor").unwrap().into_sync();
    println!("inserting coauthors");
    mutator.perform_all(paper_coauthors).unwrap();
}

pub fn create_reviews(backend: &mut Backend, nusers: i32, nauthors: i32) {
    // ReviewAssignment table
    // paper int,
    // username varchar(1024),
    // assign_type varchar(8), -- What is assign_type??
    let mut review_assignments: Vec<Vec<DataType>> = vec![];
    let reviews_per_reviewer = (nusers - nauthors + 1)/nauthors;
    for i in nauthors..nusers {
        for j in 0..reviews_per_reviewer {
            let v = vec![(i - nauthors + j).into(), format!("u{}",i).into(), "reg".into()];
            review_assignments.push(v);
        }
    }
    ...
    let mut reviews: Vec<Vec<DataType>> = vec![];
    for i in nauthors..nusers {
        let v = vec![
            "0".into(),
            (i - nauthors).into(),
...
pub fn create_papers(backend: &mut Backend) {
    ...
    let review_assignments: Vec<Vec<DataType>> = vec![
        ...
        vec![2.into(), "lara".into(), "blahblah".into()],
...
pub fn dump_query(backend: &mut Backend, query: &str, iterate:i32) {
    let mut get = backend
        .g
        .view(query)
        .unwrap()
    ...
    if iterate > 0 {
        for i in 1..iterate + 1 {
            println!(
                "{}, lookup on key {}: {:?}",
                query, i, get.lookup(&[i.into()], true)
            );
        }
        
    } else {
        // 0 is bogo key for id
        println!("{}, bogokey lookup: {:?}", query, get.lookup(&[0.into()], true));
    }
}
