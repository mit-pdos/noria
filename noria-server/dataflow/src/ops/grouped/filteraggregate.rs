    ...
    fn description(&self, detailed: bool) -> String {
        if !detailed {
            return String::from(match self.op {
                ...
                FilterAggregation::SUM => "𝛴",
            });
        }

        let op_string = match self.op {
            FilterAggregation::COUNT => "|*|".into(),
            FilterAggregation::SUM => format!("𝛴({})", self.over),
        };
        let group_cols = self
            .group
            .iter()
...
mod tests {
    ...
    fn setup(mat: bool) -> ops::test::MockGraph {
        ...
        g.set_op(
            "identity",
            &["x", "ys"],
            FilterAggregation::COUNT.over(s.as_global(), 1, &[0]),
            mat,
        );
        g
    }
    ...
    fn setup_multicolumn(mat: bool) -> ops::test::MockGraph {
        ...
        g.set_op(
            "identity",
            &["x", "z", "ys"],
            FilterAggregation::COUNT.over(s.as_global(), 1, &[0, 2]),
            mat,
        );
        g
    }
    ...
    fn it_describes() {
        ...
        let c = FilterAggregation::COUNT.over(s, 1, &[0, 2]);
        assert_eq!(c.description(true), "|*| γ[0, 2]");

        let s = FilterAggregation::SUM.over(s, 1, &[2, 0]);
